...
class Pretsa_star(Pretsa):
    ...
    def __init__(self, eventLog, greedy=True):
        ...
        self._queue = list()
        self._minDistanceMatrix, self.__minClosestSequenceMatrix = self._calculateMinDistances(
    ...
    def runPretsa(self, k, t):
        ...
        while True:
            ...
            if self.__stateIsNew(caseToSequenceDict, changedCases):
                violatingCases, violatingVariants = self._getViolatingCases(
                    tree, k, caseToSequenceDict)
                print(len(violatingVariants), end=" ")
                if len(violatingCases) == 0 and currentCost < bestOption:
                    bestOption = currentCost
                    bestTree = tree
                    bestChangedCases = changedCases
                self._updateQueue(k, tree, violatingCases, violatingVariants,
                                  currentCost, changedCases, caseToSequenceDict)
        ...
        self._tree = self._addDifferentialPrivateNosieToEnsureTCloseness(
            bestTree, t)
    ...
    def _updateQueue(self, k, tree, violatingCases, violatingVariants, currentCost, changedCases, caseToSequenceDict):
        for variant in violatingVariants.values():
            self._addOperationsToFixVariantToQueue(
                variant, k, tree, violatingCases, currentCost, changedCases.copy(), caseToSequenceDict)
        if self.__greedy:
            self._queue = sorted(self._queue, key=lambda k: (
                k["cost"], -k["isViolating"]))
        else:
            self._queue = sorted(self._queue, key=lambda k: (
                k["cost"], -len(k["changedCases"])))
    ...
    def _performOperation(self, tree, operation):
        node = find(tree, lambda node1: node1.sequence ==
                    operation[self.__operationDictCaseOrigin])
        self._cutCasesOutOfTreeStartingFromNode(
            node, operation[self.__operationDictCutOutTraces], tree)
        ...
        for case in operation[self.__operationDictCutOutTraces]:
            self._addCaseToTree(
                case, operation[self.__operationDictCasesGoal], tree)
        return tree
    ...
    def _calculateMinDistances(self, distanceMatrix):
        ...
        for sequence in distanceMatrix.keys():
            minDistanceMartrix[sequence] = min(
                distanceMatrix[sequence].values())
                ...
                if len(node.cases) < k:
                    ...
                    for newcase in newcases:
                        variant = variants.get(
                            caseToSequenceDict[newcase], dict())
                        variant[self.__variantDictCounterName] = variant.get(
                            self.__variantDictCounterName, 0) + 1
                        variant[self.__variantDictCasesSetName] = variant.get(
                            self.__variantDictCasesSetName, set())
    ...
    def _calculateDistanceHeuristic(self, k, allVariantsInTree, violatingVariants):
        ...
        conformingVariants = allVariantsInTree.difference(
            violatingVariants.keys())
        for variant in violatingVariants:
            distanceHeuristic += min(self.__costClosestViolatingSeqeunce(k, violatingVariants.keys(), variant,
                                     violatingVariants[variant]), self.__costClosestConformingSequence(conformingVariants, variant, violatingVariants[variant]))
    ...
    def __costClosestViolatingSeqeunce(self, k, violatingVariants, variantToFix, casesInVariantToFix):
        ...
        result = (minDistance * min(casesInVariantToFix,
                  abs(casesInVariantToFix - k))) / 2
        return result
    ...
    def __getViolatingVariants(self, caseToSequenceDict, violatingCases):
        ...
        for case in violatingCases:
            violatingVariants[caseToSequenceDict[case]] = violatingVariants.get(
                caseToSequenceDict[case], 0) + 1
    ...
    def _isItNecassaryToCheckAllSequences(self, variantToFix):
        closestConformingVariant = self.__closestConformingSequence.get(
            variantToFix[self.__variantDictName], None)
        closestViolatingVariant = self.__closestViolatingSequence.get(
            variantToFix[self.__variantDictName], None)
    ...
    def _getPotentialTargetSequences(self, tree, violatingVariants, variantToFix, k):
        ...
        if self._isItNecassaryToCheckAllSequences(variantToFix):
            ...
            variants = set()
            variants.add(
                self.__closestViolatingSequence[variantToFix[self.__variantDictName]])
            variants.add(
                self.__closestConformingSequence[variantToFix[self.__variantDictName]])
    ...
    def _getProjectedCost(self, violatingVariants, caseToSequenceDict, fixedCases, tree, occuredCost, k):
        remainingViolatingVariants = self.__getRemainingViolatingVariants(
            violatingVariants, fixedCases, caseToSequenceDict)
        distanceHeuristic = self._calculateDistanceHeuristic(
            k, self._getAllPotentialSequencesTree(tree), remainingViolatingVariants)
    ...
    def _addOperationsToQueueInHeuristicPRETSA(self, variantToFix, bestOperationCompliant, bestOperationViolating, tree, changedCases, caseToSequenceDict):
        if bestOperationCompliant.get("targetSequence", None) is not None:
            self.__closestConformingSequence[variantToFix[self.__variantDictName]
                                             ] = bestOperationCompliant["targetSequence"]
    ...
    def _addOperationsToFixVariantToQueue(self, variantToFix, k, tree, violatingCases, pastCost, changedCases, caseToSequenceDict):
        ...
        violatingVariants = self.__getViolatingVariants(
            caseToSequenceDict, violatingCases)
        potentialTargetSequences = self._getPotentialTargetSequences(
            tree, violatingVariants, variantToFix, k)
        for targetSequence in potentialTargetSequences:
            if not self.__areSequencesTheSame(targetSequence, variantToFix[self.__variantDictName]):
                targetNode = find(
                    tree, lambda node: node.sequence == targetSequence)
                if targetNode == None:
                    continue
                fixedCases = self._getCasesFixedByOperation(
                    variantToFix, targetNode, k)
                costOfOperartion = self._getDistanceSequences(
                    variantToFix[self.__variantDictName], targetSequence) * variantToFix[self.__variantDictCounterName]
                occuredCost = costOfOperartion + pastCost
                # If the cost by operation is higher without distance metric, there is no sense in even calculating one
                if (self.__greedy and occuredCost < minCostOfCurrentBestOption) or not self.__greedy:
                    projectedCost = self._getProjectedCost(
                        violatingVariants, caseToSequenceDict, fixedCases, tree, occuredCost, k)
                    # Block operations that would create new violations -> otherwise the problem is not feasible
                    if len(targetNode.cases) >= k:
                        bestOperationCompliant = self._getNewBestOperationDict(
                            bestOperationCompliant, occuredCost, projectedCost, targetSequence)
                    else:
                        bestOperationViolating = self._addOperationWithViolatingTargetToQueue(
                            bestOperationViolating, changedCases, occuredCost, projectedCost, tree, targetSequence, variantToFix, caseToSequenceDict)
                    minCostOfCurrentBestOption = min(
                        bestOperationViolating["projectedCost"], bestOperationCompliant["projectedCost"])
        if self.__greedy:
            self._addOperationsToQueueInHeuristicPRETSA(
                variantToFix, bestOperationCompliant, bestOperationViolating, tree, changedCases, caseToSequenceDict)
        if bestOperationCompliant.get("targetSequence", None) is not None:
            self.__addOperationToQueue(bestOperationCompliant["projectedCost"], variantToFix, tree, bestOperationCompliant[
                                       "targetSequence"], bestOperationCompliant["occuredCost"], changedCases, caseToSequenceDict, False)
    ...
    def __addOperationToQueue(self, cost, variant, tree, sequence, occuredCost, changedCases, caseToSequenceDict, isViolating=True):
        ...
        step["changedCases"] = changedCases.union(
            variant[self.__variantDictCasesSetName]).copy()
    ...
    def __willOperationCreatesNewViolation(self, variantToFix, targetVariant, k, casesInVariantToFix, tree):
        ...
        while not nodeVariantToFix is None:
            ...
            if len(nodeTargetVariant.children) != 0:
                nodeTargetVariant = self.__getChildNodeForCertainActivity(
                    activitiesTargetVariant.pop(), nodeTargetVariant)
            nodeVariantToFix = self.__getChildNodeForCertainActivity(
                activitiesVariantToFix.pop(), nodeVariantToFix)
        ...
        for node in PreOrderIter(tree):
            if node != tree:
                for annotationKey in node.annotations.keys():
                    ...
                    numerator = (((t*numberOfCasesInDistribution) /
                                 numberOfCasesInNode)-1) * numberOfCasesInNode
                    ...
                    if numberOfCasesInNode != numberOfCasesInDistribution:
                        ...
                        node.annotations[annotationKey] = node.annotations[annotationKey] + \
                            np.random.laplace(scale=epsilon)
        return tree
    ...
    def _retrieveNumberOfEventsPerActivity(self, tree):
        ...
        for node in PreOrderIter(tree):
            if node != tree:
                activityCountMap[node.name] = activityCountMap.get(
                    node.name, 0) + len(node.cases)
